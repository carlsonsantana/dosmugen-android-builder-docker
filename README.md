# DOS MUGEN Android Builder on Docker

This project allows you to build **DOS M.U.G.E.N.** games for **Android** using **Docker**.
...
```sh
docker pull carlsonsantana/dosmugen-android-builder:latest
```

Or build it yourself executing the following command on the terminal:

```sh
docker build -t dosmugen-android-builder .
```
...
* `/mugen` your DOS M.U.G.E.N. game with **csdpmi*b.zip** extracted in root directory;
